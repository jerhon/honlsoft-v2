...
export function getPostBreadcrumbs(post: PostMetadata): BreadcrumbInfo[] {
  if (post.frontmatter.page) {
    return [
      { title: "Home", url: "/" },
      {
        title: capitalize(post.frontmatter.page),
        url: "/" + post.frontmatter.page,
      },
    ]
  } else {
    return [
      { title: "Home", url: "/" },
      {
        title: capitalize(post.frontmatter.type ?? ""),
        url: "/" + post.frontmatter.type,
      },
      {
        title: capitalize(post.frontmatter.title ?? ""),
        url: "/" + post.frontmatter.type + "/" + post.parent.name,
      },
    ]
  }
}
